...
dataset_modelos = tumores_entrenamiento_10

modelo_arbol <- rpart(resultado ~ sexo + provincia, 
...
```{r}
modelo_svn <- svm(resultado ~ sexo + provincia, 
                  data = dataset_modelos, 
                  kernel = 'linear',
                  cross = 2, 
                  scale = FALSE)
...
```{r}
calcular_metricas_modelo <- function(verdaderos_positivos, verdaderos_negativos, falsos_positivos, falsos_negativos) {
   # VP+VN / Total
  exactitud <- (verdaderos_positivos + verdaderos_negativos) / sum(comparacion)
  ...
  paste("* Exactitud Total del modelo: ", round(exactitud_svn * 100, digits = 0), "%")
  paste("* Sensibilidad del modelo: ", round(sensibilidad_svn * 100, digits = 0), "%")
  paste("* Precision del modelo: ", round(precision_svn * 100, digits = 0), "%")
  paste("* Especificidad del modelo: ", round(especificidad_svn * 100, digits = 0), "%")
}
...
comparacion_svn <- table(predicciones_svn, dataset_evaluacion$resultado)
...
calcular_metricas_modelo(verdaderos_positivos_svn, verdaderos_negativos_svn, falsos_positivos_svn, falsos_negativos_svn)
...
calcular_metricas_modelo(verdaderos_positivos_arbol, verdaderos_negativos_arbol, falsos_positivos_arbol, falsos_negativos_arbol)
